...
public class LocalCluster implements Runnable {
	static final Logger log = LogManager.getLogger(LocalCluster.class);
	
	AtomicBoolean quit;
	
	String theTopology;
	
	Map<String, List<IRichBolt>> boltStreams;
	Map<String, List<IRichSpout>> spoutStreams;
	
	Map<String, IStreamRouter> streams;
	...
	public LocalCluster(CrawlerConfig config) {
		this.quit = new AtomicBoolean(false);
		this.boltStreams = new HashMap<>();
		this.spoutStreams = new HashMap<>();
		this.streams = new HashMap<>();
		this.executor = Executors.newFixedThreadPool(config.threadNum);
		this.taskQueue =  new ConcurrentLinkedQueue<>();
	}
	

	public TopologyContext submitTopology(String name, CrawlerConfig config, 
			Topology topo, CrawlerStorage storage) {
		theTopology = name;
		
		context = new TopologyContext(topo, taskQueue, storage);
	 ...
	 * @param topo Topology
	 */
	private void createSpoutInstances(Topology topo, CrawlerConfig config) {
	 ...
	 * @param topo Topology
	 */
	private void createBoltInstances(Topology topo, CrawlerConfig config) {
	 ...
	 * @param config
	 */
	private void createRoutes(Topology topo, CrawlerConfig config) {
